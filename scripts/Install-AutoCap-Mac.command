...
show_error() {
  /usr/bin/osascript - "$1" "AutoCap Installer" <<'APPLESCRIPT'
on run argv
...
button returned of (display dialog "Welcome to AutoCap $VERSION.\n\nBefore installation:\n1. Save your Premiere Pro project.\n2. Close Adobe Premiere Pro.\n3. Click Install to continue.\n\nThe panel will be installed for the current macOS account." with title "AutoCap Installer" buttons {"Cancel", "Install"} default button "Install" cancel button "Cancel")
APPLESCRIPT
) || exit 0
...
[ -f "$SOURCE_ROOT/CSXS/manifest.xml" ] || fail "The AutoCap panel folder was not found. Extract the complete ZIP and keep this installer beside the AutoCap folder."
...
if [ -d "$CACHE_ROOT" ]; then
  for CACHE_PATH in "$CACHE_ROOT"/PPRO_*_com.autocap.premiere.cep.panel "$CACHE_ROOT"/PPRO_*_com.sinhalatype.premiere.cep.panel; do
    [ -d "$CACHE_PATH" ] || continue
    case "$CACHE_PATH" in
      "$CACHE_ROOT"/PPRO_*_com.autocap.premiere.cep.panel|"$CACHE_ROOT"/PPRO_*_com.sinhalatype.premiere.cep.panel) /bin/rm -rf -- "$CACHE_PATH" ;;
    esac
  done
fi

show_message "AutoCap $VERSION was installed successfully.\n\nNext steps:\n1. Start Premiere Pro.\n2. Open Window > Extensions (Legacy) > AutoCap CEP." "AutoCap Installer"
